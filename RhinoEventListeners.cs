...
namespace LayerStalker
{
    internal class RhinoEventListeners
    {
        ...
        public void WriteLayers()
        {
            ...
            foreach (var layer in Rhino.RhinoDoc.ActiveDoc.Layers)
            {
                if (!layer.IsDeleted)
                {
                    sLayers.Add(new SerializeableLayer()
                    {
                        ...
                        LayerColor = ColorTranslator.ToHtml(layer.Color),
                        IsExp = layer.IsExpanded,
                        IsLocked = layer.IsLocked,
                        IsVisible = layer.IsVisible,
                        Predecessors = getAllChildren(Rhino.RhinoDoc.ActiveDoc.Layers.CurrentLayer),
                        Size = 500,
                        //Objects = Rhino.RhinoDoc.ActiveDoc.Objects.FindByLayer(layer),
                        CurrLayer = Rhino.RhinoDoc.ActiveDoc.Layers.CurrentLayer.Name, // Is there a more elegant way to pass this variable to the html interface only once? 
            ...
            var json = JsonConvert.SerializeObject(sLayers);
            string currLayer = JsonConvert.SerializeObject(Rhino.RhinoDoc.ActiveDoc.Layers.CurrentLayer.Name);

            if (null != LayerStalkerPlugIn.Instance.UserControl)
                LayerStalkerPlugIn.Instance.UserControl.UpdateLayers(json, currLayer);
